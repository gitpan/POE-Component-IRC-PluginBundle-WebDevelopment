...
fetch HTML code from URIs, do syntax highlighting using L<Syntax::Highlight::HTML> and
pastebin the result.
...
C<POE::Component::IRC::Plugin::Syntax::Highlight::HTML> object suitable to be
...
coloring the highlighted HTML code. B<Defaults to:> C<1>
...
section for more information. B<Defaults to:> C<irc_html_highlighter>
...
B<Defaults to:> C<qr/^highlight\s*html\s+(?=\S)/i>
...
(it defaults to C<irc_html_highlighter>) will receive input
...
The C<uri> key will contain the URI that was accessed to get HTML code (or error message)

=head3 C<error>

    { 'error' => '404 Not Found' }

If an error occurred during the retrieval of HTML code then the C<error> key will be present
...
L<POE::Component::IRC>, L<Syntax::Highlight::HTML>
